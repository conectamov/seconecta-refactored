...
function AuthModal({ open, reason, linkingProvider, onClose, onAuthenticated }: { open: boolean; reason: AuthenticationReason; linkingProvider: AuthenticationProviderId | null; onClose: () => void; onAuthenticated: (provider: AuthenticationProviderId, identifier: string, returning: boolean) => void }) {
  ...
  useEffect(() => {
    ...
    setPhone("");
    setReturning(false);
    setStage(linkingProvider === "whatsapp" ? "phone" : linkingProvider === "google" ? "google" : "choice");
  }, [linkingProvider, open]);

  useEffect(() => {
    if (!open || stage !== "google") return;
    const timeout = window.setTimeout(() => complete("google", "conta-google@mock.seconecta", false), 1350);
  ...
  const submitPhone = (event: FormEvent<HTMLFormElement>) => {
    ...
    setStage("waiting");
    let known: string[] = [];
    try { known = JSON.parse(window.localStorage.getItem(KNOWN_IDENTITIES_KEY) ?? "[]") as string[]; } catch { known = []; }
    window.setTimeout(() => complete("whatsapp", normalized, known.includes(normalized)), 1850);
  };
...
export function AuthenticationProvider({ children }: { children: React.ReactNode }) {
  ...
  const [linkingProvider, setLinkingProvider] = useState<AuthenticationProviderId | null>(null);
  ...
  const openAuthentication = (nextReason: AuthenticationReason, onAuthenticated?: () => void, preferredProvider?: AuthenticationProviderId) => {
    ...
    setLinkingProvider(preferredProvider ?? null);
    setModalOpen(true);
  };
  ...
  const finishAuthentication = (provider: AuthenticationProviderId, identifier: string) => {
    ...
    if (provider === "whatsapp") {
      let known: string[] = [];
      try { known = JSON.parse(window.localStorage.getItem(KNOWN_IDENTITIES_KEY) ?? "[]") as string[]; } catch { known = []; }
      window.localStorage.setItem(KNOWN_IDENTITIES_KEY, JSON.stringify([...new Set([...known, identifier])]));
    }
    setModalOpen(false);
  ...
  const value = useMemo<AuthenticationContextValue>(() => ({
    ...
    session,
    openAuthentication,
    connectAccount: (provider) => openAuthentication("settings", undefined, provider),
  }), [ready, session]);

  return <AuthenticationContext.Provider value={value}>{children}<AuthModal open={modalOpen} reason={reason} linkingProvider={linkingProvider} onClose={() => { setModalOpen(false); continuationRef.current = undefined; setLinkingProvider(null); }} onAuthenticated={(provider, identifier) => finishAuthentication(provider, identifier)} /></AuthenticationContext.Provider>;
}
