...
function OpportunityFactRail({ opportunity }: { opportunity: OpportunityDetail }) {
  ...
  return <div className="mt-8 grid max-w-3xl grid-cols-2 gap-2 sm:grid-cols-3">{facts.map(({ label, value, icon: Icon, tone }) => <div className="rounded-[15px] border border-white/80 bg-white/75 px-3.5 py-3.5 shadow-[0_8px_22px_rgba(28,54,43,.035)]" key={label}><Icon size={15} className={tone} /><span className="mt-3 block text-[8px] font-bold uppercase tracking-[.1em] text-[#89938e]">{label}</span><strong className="mt-1 block text-[11px] leading-4 text-[#1c372c]">{value}</strong></div>)}</div>;
}
...
export function OpportunityWorkspace({ opportunity }: { opportunity: OpportunityDetail }) {
  ...
  return <section className="mx-auto grid w-[min(1160px,calc(100%-48px))] gap-6 py-8 md:grid-cols-[230px_minmax(0,1fr)] md:items-start md:py-10">
    <aside className="sticky top-[72px] z-30 -mx-6 overflow-x-auto border-y border-[#dfe5e1] bg-[#fafbf9]/95 px-6 py-3 backdrop-blur-xl md:top-[96px] md:mx-0 md:overflow-visible md:rounded-[22px] md:border md:bg-white md:p-3" aria-label="Seções da oportunidade">
      <span className="mb-2 hidden px-3 pt-2 text-[9px] font-bold uppercase tracking-[.14em] text-[#8a948f] md:block">Nesta oportunidade</span>
      <nav className="flex min-w-max gap-2 md:grid md:min-w-0 md:gap-1">{detailSections.map(({ id, label, description, icon: Icon }) => <button className={activeSection === id ? "flex items-center gap-3 rounded-[14px] bg-[#eaf8f2] px-4 py-3 text-left text-[#056e57] md:w-full" : "flex items-center gap-3 rounded-[14px] px-4 py-3 text-left text-[#69756f] transition-colors hover:bg-[#f3f5f2] hover:text-[#1c372c] md:w-full"} type="button" onClick={() => setActiveSection(id)} aria-current={activeSection === id ? "page" : undefined} key={id}><Icon size={16} className="shrink-0" /><span><strong className="block text-[10px] font-semibold">{label}</strong><small className="mt-0.5 hidden text-[8px] opacity-70 md:block">{description}</small></span></button>)}</nav>
      <div className="mt-3 hidden border-t border-[#e5e9e6] p-3 md:block"><span className="text-[9px] font-bold uppercase tracking-[.1em] text-[#b35a42]">Prazo</span><strong className="mt-1 block text-sm text-[#1c372c]">{opportunity.deadline}</strong><p className="mt-1 text-[9px] text-[#b35a42]">{opportunity.deadlineNote}</p><div className="mt-4"><OpportunityActions opportunity={opportunity} /></div></div>
    </aside>
